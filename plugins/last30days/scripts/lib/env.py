...
def _find_project_env() -> Path | None:
    ...
    Searches for .codex/last30days.env or .claude/last30days.env in each parent
    directory,
    ...
    for parent in [cwd, *cwd.parents]:
        for dirname in ('.codex', '.claude'):
            candidate = parent / dirname / 'last30days.env'
            if candidate.exists():
                return candidate
...
def get_config() -> dict[str, Any]:
    ...
    Priority (highest wins):
      ...
      2. .codex/last30days.env or .claude/last30days.env (per-project config)
