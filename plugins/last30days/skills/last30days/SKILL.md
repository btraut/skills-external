...
## Setup: choose a Python 3.12+ runtime

Treat relative paths in this file as relative to this `SKILL.md`. The main runtime lives at `../../scripts/last30days.py`.
...
```bash
"${LAST30DAYS_PYTHON}" ../../scripts/last30days.py $ARGUMENTS --emit=compact
```

## Useful commands

```bash
"${LAST30DAYS_PYTHON}" ../../scripts/last30days.py $ARGUMENTS --emit=json
"${LAST30DAYS_PYTHON}" ../../scripts/last30days.py $ARGUMENTS --quick
"${LAST30DAYS_PYTHON}" ../../scripts/last30days.py $ARGUMENTS --deep
"${LAST30DAYS_PYTHON}" ../../scripts/last30days.py $ARGUMENTS --search=reddit,x,grounding
"${LAST30DAYS_PYTHON}" ../../scripts/last30days.py $ARGUMENTS --store
"${LAST30DAYS_PYTHON}" ../../scripts/last30days.py --diagnose
```
...
If the topic could have its own X/Twitter account (people, brands, products, companies), use the available web search tool to look for their handle with a query like:
```
{TOPIC} X twitter handle site:x.com
```
